...
<body>
	<div id="wrap">
		<div class="title">
			<img src="images/back.png" class="back" onclick="goBack()" />绑定QQ
		</div>
		<ul class="content">
			<li>
				<span class="contentLeft">QQ号码</span>
				<span class="contentLeft_2">
					<input type="text" name="accountQQ" id="accountQQ" value="${accountInfo.accountQQ }" placeholder="输入QQ号码" />
				</span>
			</li>
			<!-- <li>
		...
		<div id="bind" onclick="bindQQ()" class="tijiao_s">确定</div>
	</div>
	...
	<script type="text/javascript">
		...
		//绑定QQ
		function bindQQ(){
			var accountQQ = $("#accountQQ").val();
			if(!objIsNullOrEmpty(accountQQ)){
				alert('请填QQ号');
				return;
			}
			$.ajax({
				url:"<%=basePath%>accountinfo/bindQQ",
				type : "POST",
				data : {
					accountQQ : accountQQ
				},
