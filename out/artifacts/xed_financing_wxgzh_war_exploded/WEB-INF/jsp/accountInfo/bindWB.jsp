...
<body>
	<div id="wrap">
		<div class="title">
			<img src="images/back.png" class="back" onclick="goBack()" />绑定微博
		</div>
		<ul class="content">
			<li>
				<span class="contentLeft">微博账号</span>
				<span class="contentLeft_2">
					<input type="text" name="accountWB" id="accountWB" value="${accountInfo.accountWB }" placeholder="输入微博账号" />
				</span>
			</li>
			<!-- <li>
		...
		<div id="bind" onclick="bindWB()" class="tijiao_s">确定</div>
	</div>
	...
	<script type="text/javascript">
		...
		//绑定微博
		function bindWB(){
			var accountWB = $("#accountWB").val();
			if(!objIsNullOrEmpty(accountWB)){
				alert('请填微博号');
				return;
			}
			$.ajax({
				url:"<%=basePath%>accountinfo/bindWB",
				type : "POST",
				data : {
					accountWB : accountWB
				},
