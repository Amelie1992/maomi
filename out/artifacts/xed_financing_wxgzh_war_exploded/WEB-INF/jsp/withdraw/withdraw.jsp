		...
		<div class="wrap">
			...
			<div class="tixian">
				<p class="tixianBox">
					...
					<input type="text" name="money" id="money" value=""  placeholder="请输入提现金额，单笔最低50元" onkeyup="checkNext()"/>
				</p>
				<p class="tixianTips">
					<span>可用余额:</span>
					<span>${capitalBean.availableBalance }元</span>
					<a onclick="allIn()">全部提现</a>
				</p>
				<!-- <p class="tixianCost"><span>本次提现暂不收取手续费</span></p> -->
				<c:choose>
					<c:when test="${freeWithdrawalsNumber>0}">
						<p class="tixianCost">
							<span>免费提现次数</span>
							<span>${freeWithdrawalsNumber}次</span>
						</p>
					</c:when>
					<c:when test="${couponCount>0}">
						<p class="tixianCost">
							<span><img src="images/wx-tqtx-select-before.png" id="cost" style="width: 20px;position: relative;top: 5px;left: -3px;" onclick="checkCost(this)"/>免费提现券</span>
			...
			<a class="txBtn"  id="next">下一步</a>
			
			<input type="hidden" id="hidMoney" value="${capitalBean.availableBalance }"/>
			...
			<div class="textbox">
				<p>温馨提示</p>
				<p>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;本次提现平台收取2元手续费,从提现金额中扣除。免费提现特权和免费提现券可免除手续费。</p>
				<p>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;根据合作银行相关规定,每日14:00之前不支持提现。</p>
				<p>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;每日14:00~15:00申请提现，若当天是工作日则当日到账；若当天是周末或法定节假日，则延迟至工作日到账；15:00后申请提现，下一个工作日到账(法定节假日顺延)。</p>
				<p>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;例如投资者2017年10月3号申请"转至银行卡"，由于10月1号~8号为法定节假日，投资者资金9号到账。</p>
				<p>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;v4及以上用户一月内可使用特权提现，在工作日，9：00~17：30申请提现,当日到账。</p>
			</div>
		</div>
	</body>
...
<script type="text/javascript">
	...
	function next(){
		...
		if(parseFloat(money) > parseFloat(hidMoney)){
			alert("提现金额不可大于可提现金额！");
			return;
		}
			...
			$.ajax({
				 ...
				 type: "POST",
				  ...
				  success: function(data){
					  if(data.hisMoney < money){
						  alert("提现金额不可大于可提现金额！");
						return;
					  }else{
						  $.ajax({
							  url: "<%=basePath%>withdraw/withdrawMon",
							  type: "POST",
							  dataType:'json',
							  data : {
