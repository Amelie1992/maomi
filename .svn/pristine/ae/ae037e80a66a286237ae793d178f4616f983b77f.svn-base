(function($) {
	...
	// 验证保留到小数两位，大于等于0的数
	jQuery.validator.addMethod("checkQuota", function(value, element) {
		// var float = /^(([1-9]\d*)|0)(\.\d{2}|\.\d{1})?$/;
		var float = /^([1-9][0-9]*(\.[0-9]{1,2})?|0\.[1-9][0-9]?|0\.0[1-9]|[0])$/;
		return this.optional(element) || (float.test(value));
	}, "<font color='#ff0000'>&nbsp;请输入正确的数值</font>");
	...
	}, "<font color='#ff0000'>&nbsp;请输入大于或等于0的整数</font>");
...
jQuery.validator.addMethod("checkEmail", function(value, element) {
    var integer = /^[\w!#$%&'*+/=?^_`{|}~-]+(?:\.[\w!#$%&'*+/=?^_`{|}~-]+)*@(?:[\w](?:[\w-]*[\w])?\.)+[\w](?:[\w-]*[\w])?$/;
    return this.optional(element) || (integer.test(value));
}, "<font color='#ff0000'>&nbsp;请输入正确的邮箱</font>");
function fmoney(s, n) {
	n = n > 0 && n <= 20 ? n : 2;
